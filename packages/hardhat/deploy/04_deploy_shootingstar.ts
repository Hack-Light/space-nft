...
/**
 * Deploys a contract named "ShootingStar" using the deployer account and
...
const deployPlanet: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  const shootingStarSVG = await deploy("ShootingStarSVG", {
    from: deployer,
    libraries: {
  ...
  // Deploy accessories
  await deploy("ShootingStar", {
    from: deployer,
    args: [deployer],
    log: true,
    libraries: {
      ShootingStarSVG: shootingStarSVG.address,
    },
...
deployPlanet.tags = ["ShootingStar"];
