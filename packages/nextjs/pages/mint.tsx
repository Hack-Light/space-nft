...
const Mint: NextPage = () => {
  ...
  const handleDownload = async () => {
    const doc = document.querySelector("#img-svg");

    if (!doc) {
      return;
    }
    // Get the SVG content as a string
    const svgContent = new XMLSerializer().serializeToString(doc);
    ...
    downloadLink.download = `space_nft.svg`;
    ...
    notification.success("Space NFT Downloaded Succesfully");
  };
      ...
      <div style={{ display: "flex" }}>
        ...
        <header>
          <div className="container text-center">
            ...
            <br />
            <br />
            <p onClick={handleDownload} className="btn btn-primary btn-large">
              {isMinting ? <Spinner /> : "Download SVG"}
            </p>
          </div>
        </header>
      </div>
